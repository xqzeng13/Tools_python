...
import os
import sys
def vessleSegment(niipath,hessianname):
    sigma_minimum = 0.2
    sigma_maximum = 3.
    ...
    result_01=hessianname+'hs_'+str(niipath).split('hessian\\')[-1]###save path
    ...
    if img_arr.max()<=0:
        ...
        new_image = nib.Nifti1Image(img_arr, a.affine)####保存为nii格式
        a=0
...
if __name__ == '__main__':
    ...
    pathname = r'E:\MRA_public\final_datasets\test\hessian\\'
    hessianname=r'E:\MRA_public\final_datasets\test\hessian\result\\'
    ...
    # niifillenamelist=[]
    # j=0
    data_list = sorted(glob(os.path.join(pathname, '*nii.gz')))
    for N in range (len(data_list)):
        data_img=data_list[N]
        hessian_out=vessleSegment(data_img,hessianname)

        print('\r[ %d / %d]' % (N+1, len(data_list)), end='')
