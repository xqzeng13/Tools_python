...
def sobel_3d(image_arr):
    data=sitk.Cast(image_arr,sitk.sitkFloat32)##转换格式
    # 提取梯度图
    # gra_filter = sitk.SobelEdgeDetectionImageFilter()
    # gra_filter=sitk.CannyEdgeDetectionImageFilter()
    # gra_filter=sitk.ZeroCrossingBasedEdgeDetectionImageFilter()
    # gra_filter=sitk.CannyEdgeDetection()
    gra_filter=sitk.MeanImageFilter()
...
imgpath=r'E:\Datasets_public\IXI_MRA45\patch\train_random(178, 66, 9)__380.npy'

image = np.load(imgpath)
pred_img = image.swapaxes(0, 2)  ###交换x,z  调整方向
pred_img=np.array(pred_img,dtype='float32')
# a = sitk.GetImageFromArray(pred_img)
pred_img = sitk.GetImageFromArray(pred_img)
sitk.WriteImage(pred_img, 'D:\\other\\1221ori.nii.gz')

# edges = canny_edges_3d(img)
a=sitk.SobelEdgeDetection(pred_img)
sitk.WriteImage(a, 'D:\\other\\1220cscob.nii.gz')

canny_sitk=sobel_3d(pred_img)
sitk.WriteImage(canny_sitk, 'D:\\other\\1203.nii.gz')
